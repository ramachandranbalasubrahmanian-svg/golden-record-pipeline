...
def main():
    ...
    with Session() as db:
        clusters = db.execute(text("SELECT * FROM entity_clusters")).mappings().all()
        clusters = [dict(c) for c in clusters]
        print(f"  Found {len(clusters)} clusters")

        all_src_rows = db.execute(text("SELECT * FROM source_records WHERE quarantined = false")).mappings().all()
        src_map = {str(r["id"]): dict(r) for r in all_src_rows}
    ...
    for cluster in clusters:
        ...
        if not recs:
            # Singleton: pick from source by external_id
            continue
        ...
        customer_id = _safe_str(recs[0].get("customer_id") or str(uuid.uuid4()))
        if not customer_id:
    ...
    # Also create singleton golden records for source records not in any cluster
    with Session() as db:
        clustered_ids = set()
        for c in clusters:
        ...
        for row in singleton_rows:
            ...
            if rid not in clustered_ids:
                cid = _safe_str(row.get("customer_id")) or str(uuid.uuid4())
                gr_simple = dict(row)
                gr_simple["confidence_score"] = 0.50
                gr_simple["source_count"] = 1
                gr_simple["winning_sources"] = {k: row.get("source_system") for k in ["first_name", "last_name", "email"]}
    ...
    with Session() as db:
        for i, (customer_id, cluster_id, gr) in enumerate(golden_records):
            fn = _safe_str(gr.get("first_name"))
            ln = _safe_str(gr.get("last_name"))
            db.execute(text("""
                INSERT INTO golden_records (
                    ...
                    :cid::uuid, :cluster_id, :fn, :ln, :full_name,
                    :dob, :email, :phone, :addr, :city, :country, :nationality,
                    :kyc_status, :kyc_tier, :kyc_verified_at, :kyc_expiry_at,
                    :risk_rating, :risk_score, :is_pep, :pep_type, :pep_detected_at,
                    :is_sanctioned, :sanctions_list, :sanctions_detected_at,
                    :conf, :src_count, :winning::jsonb, NOW(), NOW()
            ...
            """), {
                ...
                "fn": fn,
                "ln": ln,
                "full_name": _safe_str(gr.get("full_legal_name") or f"{fn or ''} {ln or ''}".strip()),
                "dob": _safe_str(gr.get("date_of_birth")),
                "email": _safe_str(gr.get("email")),
                "phone": _safe_str(gr.get("phone")),
                "addr": _safe_str(gr.get("address_line1")),
                "city": _safe_str(gr.get("city")),
                "country": _safe_str(gr.get("country")),
                "nationality": _safe_str(gr.get("nationality")),
                "kyc_status": _safe_str(gr.get("kyc_status")),
                "kyc_tier": _safe_str(gr.get("kyc_tier")),
                "kyc_verified_at": _safe_str(gr.get("kyc_verified_at")),
                "kyc_expiry_at": _safe_str(gr.get("kyc_expiry_at")),
                "risk_rating": _safe_str(gr.get("risk_rating")),
                "risk_score": float(gr.get("risk_score") or 0),
                "is_pep": bool(gr.get("is_pep")),
                "pep_type": _safe_str(gr.get("pep_type")),
                "pep_detected_at": _safe_str(gr.get("pep_detected_at")),
                "is_sanctioned": bool(gr.get("is_sanctioned")),
                "sanctions_list": _safe_str(gr.get("sanctions_list")),
                "sanctions_detected_at": _safe_str(gr.get("sanctions_detected_at")),
    ...
    print(f"Writing lineage entries...")
    with Session() as db:
        count = 0
            ...
            for entry in lineage:
                db.execute(text("""
                    INSERT INTO attribute_lineage (
                        ...
                        gen_random_uuid(), :cid::uuid, :attr, :val, :src,
                        :rule, :conf, :lock, :competing::jsonb, NOW()
                ...
                """), {
                    ...
                    "val": _safe_str(entry.get("winning_value")),
                    ...
                    "competing": json.dumps(entry.get("competing_sources") or []),
                })
                count += 1
            if count % 5000 == 0:
                db.commit()
    ...
    print(f"Survivorship wins by source: " + " | ".join(f"{s}: {c}" for s, c in sorted(source_wins.items())))
